((ckan: ICkan) => {
    class Base extends EventTarget implements CkanextFiles.Adapter {
        static defaultSettings: Record<string, any> = {};

        defaultSettings: Record<string, any> = {};
        settings: CkanextFiles.UploaderSettings;
        sandbox: ISandbox;
        csrfToken: string;

        constructor(settings?: Record<string, any>, ...rest: any) {
            super();
            this.sandbox = ckan.sandbox();

            this.settings = {
                ...this.sandbox.files.defaultSettings,
                ...(this.constructor as CkanextFiles.AdapterConstructor)
                    .defaultSettings,
        ...
        upload(file: File, params: Record<string, any>) {
    ...
    class Multipart extends Base {
        ...
        async _doUpload(file: File, info: CkanextFiles.adapters.UploadInfo) {
            let start = info.storage_data["uploaded"] || 0;
            const keys = Object.keys(info.storage_data["parts"] || {}).map(
                (k) => Number(k),
            );
        ...
        _showUpload(id: string): Promise<CkanextFiles.adapters.UploadInfo> {
            return new Promise((done, fail) =>
        ...
        _uploadChunk(
            info: CkanextFiles.adapters.UploadInfo,
            upload: Blob,
            part: number,
            extras: any = {},
        ): Promise<CkanextFiles.adapters.UploadInfo> {
            ...
            const result = new Promise<CkanextFiles.adapters.UploadInfo>(
                (done, fail) => {
            ...
            request.open(
                "POST",
                this.sandbox.client.url("/api/action/files_multipart_update"),
            );
        ...
        _completeUpload(
            ...
            return new Promise((done, fail) =>
                this.sandbox.client.call(
                    ...
                    Object.assign({}, this.settings.completePayload || {}, {
                        id: info.id,
                    }),
                    (data: any) => {
                        done(data.result);
