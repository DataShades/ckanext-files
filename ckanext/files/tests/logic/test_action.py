...
class TestFileRegister:
    ...
    def test_missing_analyze_capability(self, faker: Faker):
        ...
        with pytest.raises(tk.ValidationError):
            call_action("files_file_register", location=faker.file_name())

    def test_already_registered(self, file: dict[str, Any]):
        """If file is registered in the system it causes an error."""
        with pytest.raises(tk.ValidationError):
            call_action("files_file_register", location=file["location"])
...
class TestFileSearch:
    def test_minimal_search(self, file_factory: types.TestFactory):
        ...
        result = call_action("files_file_search")
    ...
    def test_sort(self, file_factory: types.TestFactory, faker: Faker):
        ...
        result = call_action("files_file_search", sort="name")
        assert [f["id"] for f in result["results"]] == [first["id"], second["id"]]

        result = call_action("files_file_search", sort="size")
        assert [f["id"] for f in result["results"]] == [second["id"], first["id"]]

        result = call_action("files_file_search", sort=[["size", "desc"]])
        assert [f["id"] for f in result["results"]] == [first["id"], second["id"]]

        result = call_action("files_file_search", sort=[["size", "asc"]])
        assert [f["id"] for f in result["results"]] == [second["id"], first["id"]]

        result = call_action("files_file_search", sort=[["owner_id", "desc"], "size"])
        assert [f["id"] for f in result["results"]] == [second["id"], first["id"]]

        result = call_action("files_file_search", sort=[["owner_id", "asc"], "size"])
        assert [f["id"] for f in result["results"]] == [second["id"], first["id"]]

        result = call_action("files_file_search", sort=["owner_id", ["size", "desc"]])
        assert [f["id"] for f in result["results"]] == [first["id"], second["id"]]
    ...
    def test_filter_by_value(self, faker: Faker, file_factory: types.TestFactory):
        ...
        result = call_action("files_file_search", filters={})
        assert result["count"] == 2

        result = call_action("files_file_search", filters={"size": 10})
        assert result["results"] == [small]

        result = call_action("files_file_search", filters={"size": {"$gte": 50}})
        assert result["results"] == [big]

        result = call_action("files_file_search", filters={"owner_type": {"$eq": "user"}})
    ...
    def test_filter_by_data(self, file_factory: types.TestFactory):
        ...
        big = call_action("files_file_show", id=big_obj.id)
        small = call_action("files_file_show", id=small_obj.id)

        result = call_action("files_file_search", filters={"storage_data": {"size": 10}})
        assert result["results"] == [small]

        result = call_action("files_file_search", filters={"storage_data": {"size": {"$gte": 50}}})
        assert result["results"] == [big]

        result = call_action("files_file_search", filters={"storage_data": {"big": {"$ne": None}}})
        assert big in result["results"]
        assert small in result["results"]

        result = call_action("files_file_search", filters={"storage_data": {"big": True}})
        assert result["results"] == [big]

        result = call_action("files_file_search", filters={"storage_data": {"name": None}})
        assert result["results"] == [small]

        result = call_action("files_file_search", filters={"storage_data": {"name": "big"}})
        assert result["results"] == [big]
...
class TestFileShow:
    def test_show_missing(self, faker: Faker):
        ...
        with pytest.raises(tk.NotFound):
            call_action("files_file_show", id=faker.uuid4())
...
class TestFileRename:
    def test_rename_missing(self, faker: Faker):
        ...
        with pytest.raises(tk.NotFound):
            call_action("files_file_rename", id=faker.uuid4(), name=faker.file_name())

    def test_rename_real(self, faker: Faker, file: dict[str, Any]):
        """Real file can be renamed."""
        name = faker.file_name()
        result = call_action("files_file_rename", id=file["id"], name=name)
    ...
    def test_name_secured(self, file: dict[str, Any]):
        ...
        result = call_action("files_file_rename", id=file["id"], name=bad_name)
...
class TestFilePin:
    def test_pin_missing(self, faker: Faker):
        ...
        with pytest.raises(tk.NotFound):
            call_action("files_file_pin", id=faker.uuid4())
    ...
    def test_pin_real(self, file: dict[str, Any]):
        ...
        result = call_action("files_file_pin", id=file["id"])
...
class TestFileOwnershipTransfer:
    ...
    def test_transfer_expires_owner(self, faker: Faker, file_factory: types.TestFactory):
...
class TestFileOwnerScan:
    def test_filter_by_owner(self, file_factory: types.TestFactory, user: dict[str, Any]):
        ...
        result = call_action("files_file_scan", owner_type="user", owner_id=user["id"])
        assert result["results"] == [owned]

        result = call_action("files_file_scan", owner_type="user", owner_id=fake.unique.uuid4())
